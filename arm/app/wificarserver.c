...
#define MyUart2DrivesName "/dev/Uart2ForGSM"
#define DATALENGTH 100

#define IPhoneNumber 13572990657

Stm32SeniorData GetStmData;
unsigned char CmdToStm32;
int fd_uart_to_32;
int fdForGSM;
unsigned char sendD = 0X1A;
int GSMCount = 1;
/* 事件 */
#define QT_Request 1
...
/* 发送数据 */
int SendToGSM(unsigned char *msg, int length){
	int len;
	printf("send msg is %s\n",msg);
	len = write(fdForGSM, msg, length);
...
void fasync_uart_read(int signum){
	...
	if(GetStmData.FireDataHigh > TH_FIRE){
		...
		if((GSMCount == 1)||(GSMCount == 2)){
			...
			SendToGSM("AT+COPS?\r\n",10);
			sleep(1);
			SendToGSM("AT+CSCS=\"GSM\"\r\n",15);
			sleep(1);
			SendToGSM("AT+CMGF=1\r\n",11);
			sleep(1);
			SendToGSM("AT+CMGS=\"13572990657\"\r\n",23);
			sleep(3);
			SendToGSM("YangYang,512Lab is firing!\r\n",28);
			sleep(1);
			SendToGSM(&sendD,1);
			sleep(1);
...
/* 异步读数据 */
void fasync_uart2_read(int signum){
	#if 0
	//read(fd, &Data, 8);
	int length;
	int i;
	memset(GetFromGSMData, 0x0, 10);
	tcflush(fdForGSM, TCIOFLUSH);
	sleep(1);
	length = read(fdForGSM, GetFromGSMData, 10);
	//write(fdForGSM, GetFromGSMData, 10);
	//GetFromGSMData[20] = '\0';
	//printf("data %s\n",GetFromGSMData);
	for(i = 0; i < length; i++){
		printf("length is %d :%d %c\n",length,i,GetFromGSMData[i]);	
	}
	printf("\n");
	#endif
}
...
int Uart2Set(void){
	int ret;
	fdForGSM = open(MyUart2DrivesName, O_RDWR);  
	if(fdForGSM < 0){  
	    printf("Error: open /dev/uart1 error!/n");  
	    return 1;  
	} 
	ret = ioctl(fdForGSM, BSP9600, 9600);
...
/* 异步机制设置 */
void FasyncSet2(void){
	int oflag;
	signal(SIGIO, fasync_uart2_read);
	fcntl(fdForGSM, F_SETOWN,getpid());
	oflag = fcntl(fdForGSM, F_GETFL); 
	fcntl(fdForGSM, F_SETFL, oflag | FASYNC);
}
